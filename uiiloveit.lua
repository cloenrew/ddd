...
function library:addDropdown(options) 
    ...
    if options.default == "" or options.default == nil then
        cfg.default = cfg.multi and {} or ""
    else
    ...
    -- Functions 
        ...
        function cfg.set(value) 
            ...
            -- Handle empty value
            if value == "" or (isTable and #value == 0) then
                text.Text = ""
...
outline.MouseButton1Down:Connect(function()
    ...
    cfg.binding = library:connection(uis.InputBegan, function(input, game_event)  
        ...
        if inputType.Name ~= "Keyboard" then
            keyName = tostring(inputType):match("Enum%.UserInputType%.(.+)") or "None"
        else
            keyName = tostring(key):match("Enum%.KeyCode%.(.+)") or "None"
        end
        
        text_label.Text = "[ " .. keyName .. " ]"
        cfg.set(inputType.Name ~= "Keyboard" and inputType or key)
...
outline.MouseButton2Click:Connect(function()
    local context_menu = library:create("Frame", {
        Parent = library.gui,
        Name = "",
        Visible = true,
        Position = dim_offset(outline.AbsolutePosition.X, outline.AbsolutePosition.Y + outline.AbsoluteSize.Y + 2),
        BorderColor3 = rgb(0, 0, 0),
        Size = dim2(0, 60, 0, 0),
        BorderSizePixel = 0,
        AutomaticSize = Enum.AutomaticSize.Y,
    ...
    for i, mode in pairs(modes) do
        local option = library:create("TextButton", {
            ...
            Name = "",
            FontFace = library.font,
            TextColor3 = cfg.mode == mode and themes.preset.accent or rgb(180, 180, 180),
            BorderColor3 = rgb(0, 0, 0),
            Text = mode,
            BackgroundTransparency = 1,
            Size = dim2(1, 0, 0, 12),
        ...
        option.MouseButton1Click:Connect(function()
            for i, child in pairs(background:GetChildren()) do
                if child:IsA("TextButton") then
                    child.TextColor3 = rgb(180, 180, 180)
                end
            end
    ...
    close_connection = library:connection(uis.InputBegan, function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseButton2 then
            if not library:mouseInFrame(context_menu) then
                context_menu:Destroy()
...
function library:addButton(options)
    ...
    local click_overlay = library:create("Frame", {
        Parent = background,
        Name = "",
        Size = dim2(1, 0, 1, 0),
        BorderSizePixel = 0,
    ...
    library:applyTheme(click_overlay, "accent", "BackgroundColor3")
    
    library:create("UIGradient", {
        Parent = click_overlay,
        Name = "",
        Rotation = 90,
        Color = rgbseq{rgbkey(0, rgb(255, 255, 255)), rgbkey(1, rgb(180, 180, 180))}
    })
    ...
    button.MouseButton1Click:Connect(function()
        click_overlay.BackgroundTransparency = 0.7
        
        tween_service:Create(click_overlay, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            BackgroundTransparency = 1
        }):Play()
        
        tween_service:Create(background, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        ...
        task.spawn(function()
            ...
            tween_service:Create(background, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                BackgroundColor3 = rgb(19, 19, 19)
            }):Play()
        end)
    ...
    button.MouseEnter:Connect(function()
        tween_service:Create(button, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
do 
    ...
    function notifications:create_notification(options)
        local cfg = {
            message = options.name or options.message or "Notification",
        ...
        NewInd.BackgroundColor3 = Color3.fromRGB(46, 46, 46)
        NewInd.BackgroundTransparency = 0
        ...
        ActualInd.BackgroundColor3 = Color3.fromRGB(21, 21, 21)
        ActualInd.BorderColor3 = Color3.fromRGB(0, 0, 0)
        ActualInd.Size = UDim2.fromScale(1, 1)
        ActualInd.BackgroundTransparency = 0
        ...
        IndInline.BackgroundColor3 = Color3.fromRGB(21, 21, 21)
        ...
        IndInline.BackgroundTransparency = 0
        ...
        task.delay(cfg.duration + 0.1, function()
            notification:remove()
        end)
